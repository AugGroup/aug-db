...
@NamedNativeQueries({
		...
		@NamedNativeQuery(name = "SEARCH_REQUEST_BY_ID", query = "SELECT a.ID, a.REQUEST_DATE, a.REQUESTER_NAME, a.STATUS, a.APPROVAL_NAME,a.APPROVE_DATE, a.NUMBER_APPLICANT, a.SPECIFIC_SKILL,"
				+ " a.YEAR_EXPERIENCE,j.NAME as MAS_JOB_LEVEL_NAME,t.NAME as MAS_TECHNOLOGY_NAME,a.MASJOBLEVEL_ID,a.MASTECHNOLOGY_ID, j.ID, t.ID"
				+ " FROM AUG_REQUEST a LEFT JOIN MAS_JOBLEVEL j ON a.MASJOBLEVEL_ID = j.ID LEFT JOIN MAS_TECHNOLOGY t ON a.MASTECHNOLOGY_ID = t.ID WHERE a.ID = :ID", resultClass = AugRequestDto.class),
